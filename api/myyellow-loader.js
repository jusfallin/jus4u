...
<style id="myyellow-loader-style">
  ...
  #myyellow-loader {
    ...
    background:linear-gradient(145deg,#fff7f5 0%,#f8e9ef 46%,#eee5f7 100%);
    color:#4a2732; overflow:hidden;
    opacity:1; visibility:visible;
    transition:opacity 900ms cubic-bezier(.22,1,.36,1),visibility 900ms;
    font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;
  }
  #myyellow-loader::before,#myyellow-loader::after{
    content:""; position:absolute; border-radius:50%; pointer-events:none; filter:blur(42px);
    opacity:.34; animation:myyellowDrift 7s ease-in-out infinite alternate;
  }
  ...
  .myyellow-loader-kicker{font-family:Georgia,"Times New Roman",serif;font-style:italic;font-size:18px;color:#96505f;margin-bottom:7px;animation:myyellowTextIn 900ms .15s both;}
  .myyellow-loader-title{font-family:Georgia,"Times New Roman",serif;font-size:clamp(25px,7vw,34px);font-weight:500;line-height:1.15;margin:0;color:#4a2732;animation:myyellowTextIn 900ms .28s both;}
  .myyellow-loader-sub{margin:10px 0 24px;font-size:13px;letter-spacing:.03em;color:#8c6d76;min-height:20px;animation:myyellowTextIn 900ms .42s both;}
  ...
  .myyellow-loader-bar{height:100%;width:0%;border-radius:inherit;background:linear-gradient(90deg,#d78da2,#e5bf6c,#b987a8);box-shadow:0 0 16px rgba(215,141,162,.35);transition:width 100ms linear;}
...
<div id="myyellow-loader" aria-label="Loading your surprise">
  <div class="myyellow-loader-inner">
    <div class="myyellow-loader-photo-wrap">
      ...
      <img class="myyellow-loader-photo" src="/IMG_20260805_163808.jpg" alt="">
...
(function(){
  var loader=document.getElementById('myyellow-loader');
  if(!loader)return;
  var bar=loader.querySelector('.myyellow-loader-bar');
  var pct=loader.querySelector('.myyellow-loader-percent');
  var progress=loader.querySelector('.myyellow-loader-progress');
  var reduce=window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  document.documentElement.classList.add('my-yellow-loading');
  document.body.classList.add('my-yellow-loading');
  var start=performance.now(), duration=reduce?900:4200, finished=false;
  function finish(){
    if(finished)return; finished=true;
    bar.style.width='100%'; pct.textContent='100%'; progress.setAttribute('aria-valuenow','100');
    setTimeout(function(){
      document.documentElement.classList.remove('my-yellow-loading');
      document.body.classList.remove('my-yellow-loading');
      loader.classList.add('myyellow-done');
  ...
  function tick(now){
    ...
    var value=Math.floor(eased*100);
    bar.style.width=value+'%'; pct.textContent=value+'%'; progress.setAttribute('aria-valuenow',String(value));
  ...
  html = html.replace('<body>', '<body class="my-yellow-loading">' + loaderMarkup);
